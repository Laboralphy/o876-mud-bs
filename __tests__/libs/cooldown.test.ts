...
describe('CooldownManager', () => {
    ...
    describe('create', () => {
        it('starts with an empty timers array', () => {
            const cd = mgr.create({ duration: 5, charges: 3 });
        ...
        it('is active when duration >= 0', () => {
            expect(mgr.create({ duration: 0,  charges: 1 }).active).toBe(true);
            expect(mgr.create({ duration: 10, charges: 1 }).active).toBe(true);
        });

        it('is inactive when duration < 0', () => {
            expect(mgr.create({ duration: -1, charges: 1 }).active).toBe(false);
        });
    });
    ...
    describe('pushTimer', () => {
        ...
        it('does nothing when already at capacity', () => {
            const cd = mgr.create({ duration: 5, charges: 2 });
            mgr.pushTimer(cd);
            mgr.pushTimer(cd);
            mgr.pushTimer(cd); // over capacity — should be ignored
            expect(cd.timers.length).toBe(2);
        });

        it('single-charge cooldown blocks after one push', () => {
            const cd = mgr.create({ duration: 5, charges: 1 });
            mgr.pushTimer(cd);
            mgr.pushTimer(cd); // ignored
            expect(cd.timers.length).toBe(1);
        });
    });
    ...
    describe('process', () => {
        ...
        it('removes a timer that reaches 0 after one process', () => {
            const cd = mgr.create({ duration: 1, charges: 2 });
            mgr.pushTimer(cd);
            mgr.process(cd);
        ...
        it('removes a timer after exactly duration process calls', () => {
            const cd = mgr.create({ duration: 3, charges: 2 });
            mgr.pushTimer(cd);
            mgr.process(cd); // 2
            mgr.process(cd); // 1
            expect(cd.timers.length).toBe(1);
            mgr.process(cd); // 0 → removed
            expect(cd.timers.length).toBe(0);
        });

        it('does nothing to an empty timers array', () => {
            const cd = mgr.create({ duration: 5, charges: 2 });
            expect(() => mgr.process(cd)).not.toThrow();
            expect(cd.timers).toEqual([]);
        });

        it('only removes expired timers from the front', () => {
            const cd = mgr.create({ duration: 10, charges: 3 });
            mgr.pushTimer(cd); // timer A: 10
            mgr.process(cd);   // A: 9
            mgr.process(cd);   // A: 8
            mgr.pushTimer(cd); // timer B: 10
    ...
    describe('multi-charge behaviour', () => {
        it('makes a slot available again once the oldest timer expires', () => {
            const cd = mgr.create({ duration: 2, charges: 2 });
            mgr.pushTimer(cd); // use charge A
            mgr.pushTimer(cd); // use charge B — now full
            expect(cd.timers.length).toBe(2);

            mgr.process(cd); // [1, 1]
            mgr.process(cd); // [0, 0] → both removed
            expect(cd.timers.length).toBe(0);

            mgr.pushTimer(cd); // slot freed — can push again
            expect(cd.timers.length).toBe(1);
        });

        it('staggered pushes expire independently', () => {
            const cd = mgr.create({ duration: 3, charges: 3 });
            mgr.pushTimer(cd); // A: 3
            mgr.process(cd);   // A: 2
            mgr.pushTimer(cd); // B: 3  →  [2, 3]
            mgr.process(cd);   // A: 1, B: 2  →  [1, 2]
            mgr.process(cd);   // A: 0 → removed, B: 1  →  [1]
            expect(cd.timers).toEqual([1]);
            mgr.process(cd);   // B: 0 → removed
            expect(cd.timers).toEqual([]);
        });

        it('blocks all further pushes when all charges are on cooldown', () => {
            const cd = mgr.create({ duration: 10, charges: 2 });
            mgr.pushTimer(cd);
            mgr.pushTimer(cd);
        ...
        it('full cycle: 3 charges, use all, wait for recovery, reuse', () => {
            const cd = mgr.create({ duration: 2, charges: 3 });
            mgr.pushTimer(cd);
            mgr.pushTimer(cd);
            mgr.pushTimer(cd);
            expect(cd.timers.length).toBe(3); // all charges spent

            mgr.process(cd); // [1,1,1]
            mgr.process(cd); // [0,0,0] → all removed
