...
import { makeAbilityModifierEffect, makeRegenEffect } from '../helpers/helpers';
...
describe('getEffects', () => {
    ...
    it('returns effects with duration > 0', () => {
        creature.state.effects.push(makeAbilityModifierEffect({ duration: 3 }));
        expect(creature.getters.getEffects).toHaveLength(1);
    });

    it('filters out effects with duration of 0', () => {
        creature.state.effects.push(makeAbilityModifierEffect({ duration: 0 }));
        expect(creature.getters.getEffects).toHaveLength(0);
    });

    it('keeps active effects and discards expired ones', () => {
        creature.state.effects.push(makeAbilityModifierEffect({ id: 'a', duration: 5 }));
        creature.state.effects.push(makeAbilityModifierEffect({ id: 'b', duration: 0 }));
        creature.state.effects.push(makeAbilityModifierEffect({ id: 'c', duration: 1 }));
...
describe('getActiveEffects', () => {
    ...
    it('filters out effects with no registered program', () => {
        ...
        creature.state.effects.push(makeAbilityModifierEffect({ duration: 5 }));
        expect(creature.getters.getActiveEffects).toHaveLength(0);
    });

    it('only keeps active effects that have a program', () => {
        creature.state.effects.push(makeAbilityModifierEffect({ id: 'no-prog', duration: 5 }));
