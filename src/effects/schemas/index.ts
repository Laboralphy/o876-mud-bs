...
const _WrappedEffectDamageImmunity = BaseEffectSchema.extend({
    type: z.literal(CONSTS.EFFECT_DAMAGE_IMMUNITY),
    data: EffectDamageImmunity,
});
const _WrappedEffectDamageReduction = BaseEffectSchema.extend({
    type: z.literal(CONSTS.EFFECT_DAMAGE_REDUCTION),
    data: EffectDamageReduction,
});
const _WrappedEffectDamageResistance = BaseEffectSchema.extend({
    type: z.literal(CONSTS.EFFECT_DAMAGE_RESISTANCE),
    data: EffectDamageResistance,
});
const _WrappedEffectDamage = BaseEffectSchema.extend({
    type: z.literal(CONSTS.EFFECT_DAMAGE),
    data: EffectDamage,
});
const _WrappedEffectDamageVulnerability = BaseEffectSchema.extend({
    type: z.literal(CONSTS.EFFECT_DAMAGE_VULNERABILITY),
    data: EffectDamageVulnerability,
});
const _WrappedEffectHealingFactor = BaseEffectSchema.extend({
    type: z.literal(CONSTS.EFFECT_HEALING_FACTOR),
    data: EffectHealingFactor,
});
const _WrappedEffectHealingModifier = BaseEffectSchema.extend({
    type: z.literal(CONSTS.EFFECT_HEALING_MODIFIER),
    data: EffectHealingModifier,
});
const _WrappedEffectHeal = BaseEffectSchema.extend({
    type: z.literal(CONSTS.EFFECT_HEAL),
    data: EffectHeal,
});
const _WrappedEffectRegenerationSchema = BaseEffectSchema.extend({
    type: z.literal(CONSTS.EFFECT_REGENERATION),
    data: EffectRegeneration,
});
const _WrappedEffectAbilityCheckModifier = BaseEffectSchema.extend({
    type: z.literal(CONSTS.EFFECT_ABILITY_CHECK_MODIFIER),
    data: EffectAbilityCheckModifier,
});
const _WrappedEffectAbilityModifier = BaseEffectSchema.extend({
    type: z.literal(CONSTS.EFFECT_ABILITY_MODIFIER),
    data: EffectAbilityModifier,
});
const _WrappedEffectAbilityResistanceModifier = BaseEffectSchema.extend({
    type: z.literal(CONSTS.EFFECT_ABILITY_RESISTANCE_MODIFIER),
    data: EffectAbilityResistanceModifier,
});
const _WrappedEffectArmorClassModifier = BaseEffectSchema.extend({
    type: z.literal(CONSTS.EFFECT_ARMOR_CLASS_MODIFIER),
    data: EffectArmorClassModifier,
});
const _WrappedEffectAttackModifier = BaseEffectSchema.extend({
    type: z.literal(CONSTS.EFFECT_ATTACK_MODIFIER),
    data: EffectAttackModifier,
});
const _WrappedEffectExtraHitpoints = BaseEffectSchema.extend({
    type: z.literal(CONSTS.EFFECT_EXTRA_HITPOINTS),
    data: EffectExtraHitpoints,
});
const _WrappedEffectResistThreat = BaseEffectSchema.extend({
    type: z.literal(CONSTS.EFFECT_RESIST_THREAT),
    data: EffectResistThreat,
});
const _WrappedEffectSkillModifier = BaseEffectSchema.extend({
    type: z.literal(CONSTS.EFFECT_SKILL_MODIFIER),
    data: EffectSkillModifier,
});
const _WrappedEffectSpeedFactor = BaseEffectSchema.extend({
    type: z.literal(CONSTS.EFFECT_SPEED_FACTOR),
    data: EffectSpeedFactor,
});
const _WrappedEffectCharm = BaseEffectSchema.extend({
    type: z.literal(CONSTS.EFFECT_CHARM),
    data: EffectCharm,
});
const _WrappedEffectDisease = BaseEffectSchema.extend({
    type: z.literal(CONSTS.EFFECT_DISEASE),
    data: EffectDisease,
});
const _WrappedEffectFear = BaseEffectSchema.extend({
    type: z.literal(CONSTS.EFFECT_FEAR),
    data: EffectFear,
});
const _WrappedEffectParalysis = BaseEffectSchema.extend({
    type: z.literal(CONSTS.EFFECT_PARALYSIS),
    data: EffectParalysis,
});
const _WrappedEffectPetrification = BaseEffectSchema.extend({
    type: z.literal(CONSTS.EFFECT_PETRIFICATION),
    data: EffectPetrification,
});
const _WrappedEffectPoison = BaseEffectSchema.extend({
    type: z.literal(CONSTS.EFFECT_POISON),
    data: EffectPoison,
});
const _WrappedEffectStun = BaseEffectSchema.extend({
    type: z.literal(CONSTS.EFFECT_STUN),
    data: EffectStun,
});
const _WrappedEffectBlindness = BaseEffectSchema.extend({
    type: z.literal(CONSTS.EFFECT_BLINDNESS),
    data: EffectBlindness,
});
const _WrappedEffectDarkvision = BaseEffectSchema.extend({
    type: z.literal(CONSTS.EFFECT_DARKVISION),
    data: EffectDarkvision,
});
const _WrappedEffectInvisibility = BaseEffectSchema.extend({
    type: z.literal(CONSTS.EFFECT_INVISIBILITY),
    data: EffectInvisibility,
});
const _WrappedEffectLight = BaseEffectSchema.extend({
    type: z.literal(CONSTS.EFFECT_LIGHT),
    data: EffectLight,
});
const _WrappedEffectSeeInvisibility = BaseEffectSchema.extend({
    type: z.literal(CONSTS.EFFECT_SEE_INVISIBILITY),
    data: EffectSeeInvisibility,
});
const _WrappedEffectStealth = BaseEffectSchema.extend({
